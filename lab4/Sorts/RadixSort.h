...
private:
    ...
    static void sort_impl(T* arr, size_t n, KeyExtractor key_fn) {
        if (n <= 1) return;
...
public:
    template<typename T, typename KeyExtractor>
    static void sort(T* arr, size_t n, KeyExtractor key_fn) {
        sort_impl(arr, n, key_fn);
    }

    template<typename T, typename KeyExtractor>
    static void sort(std::vector<T>& vec, KeyExtractor key_fn) {
        if (!vec.empty()) sort_impl(vec.data(), vec.size(), key_fn);
    }

    template<typename T, size_t N, typename KeyExtractor>
    static void sort(T (&arr)[N], KeyExtractor key_fn) {
        sort_impl(arr, N, key_fn);
    }
    ...
    sort(T* arr, size_t n) {
        sort_impl(arr, n, [](T x) { return x; });
    }

    template<typename T>
    static typename std::enable_if<std::is_integral<T>::value, void>::type
    sort(std::vector<T>& vec) {
        if (!vec.empty()) sort_impl(vec.data(), vec.size(), [](T x) { return x; });
    }
